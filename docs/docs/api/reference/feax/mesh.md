...
Parameters
----------
- **mesh** (*Mesh*): Input mesh.  ``mesh.points`` must be ``(n_nodes, 2)`` or ``(n_nodes, 3)``; ``mesh.cells`` must be ``(n_cells, n_nodes_per_cell)`` matching ``mesh.ele_type``.
- **levels** (*int, default 1*): Number of refinement levels.  ``0`` returns ``mesh`` unchanged.
