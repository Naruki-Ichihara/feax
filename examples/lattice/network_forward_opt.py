...
class SimpleNN(nnx.Module):
    """
    Simple 3-layer fully-connected neural network with dropout regularization.
...
def loss_fn(model: SimpleNN, x_batch, y_batch, deterministic: bool):
    ...
    Args:
        ...
        deterministic: If True, disable dropout (for evaluation)
    """
    # Use nnx.vmap to properly handle RNG state in dropout layers
    predictions = nnx.vmap(
        lambda m, x: m(x, deterministic=deterministic),
        in_axes=(None, 0),  # Don't vmap over model, vmap over batch
        axis_size=x_batch.shape[0]
    )(model, x_batch)
...
def train_step(model: SimpleNN, optimizer: nnx.Optimizer, x_batch, y_batch):
    """Single training step with gradient update (with dropout enabled)."""
    # Use a wrapper to avoid keyword arguments in value_and_grad
    def train_loss(model):
        return loss_fn(model, x_batch, y_batch, deterministic=False)
...
def eval_step(model: SimpleNN, x_batch, y_batch):
    """Evaluation step (no gradient computation, dropout disabled)."""
    return loss_fn(model, x_batch, y_batch, deterministic=True)
...
def train_network(X_train, y_train, X_val, y_val,
    ...
    model = SimpleNN(input_size, hidden_sizes, output_size, rngs=nnx.Rngs(0), dropout_rate=dropout_rate)
    ...
    for epoch in range(num_epochs):
        ...
        # Validation loss (dropout automatically disabled in eval_step)
        ...
        if (epoch + 1) % 10 == 0:
            print(f"Epoch {epoch+1}/{num_epochs} - "
                  ...
                  f"(Best: {best_val_loss:.6f}, Patience: {patience_counter}/{early_stopping_patience})")

    return model, train_losses, val_losses


def evaluate_model(model: SimpleNN, X_test, y_test):
    """Evaluate model on test set (with dropout disabled)."""
    # Use nnx.vmap to properly handle RNG state
    predictions = nnx.vmap(
        lambda m, x: m(x, deterministic=True),
        in_axes=(None, 0),
        axis_size=X_test.shape[0]
    )(model, X_test)
...
def main():
    ...
    # Train network with improved hyperparameters
